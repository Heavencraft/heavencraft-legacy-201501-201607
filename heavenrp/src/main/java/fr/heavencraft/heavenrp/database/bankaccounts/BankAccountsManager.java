...
public class BankAccountsManager
{
	...
	{
		try (Connection connection = HeavenRP.getConnection();PreparedStatement ps = connection.prepareStatement(
	...
	public static List<BankAccount> getAccountByOwner(String owner) throws HeavenException
	{
		...
		try (Connection connection = HeavenRP.getConnection();
				PreparedStatement ps = connection.prepareStatement("(SELECT ba.id, ba.owner, ba.type, ba.balance " + // Sélection
																														// des
																														// comptes
																														// de
																														// villes
						"FROM bank_account ba, mayors m, users u " + "WHERE ba.type = 'T' "
						+ "AND ba.owner = m.region_name " + "AND m.user_id = u.id " + "AND u.name = ?) " + "UNION "
